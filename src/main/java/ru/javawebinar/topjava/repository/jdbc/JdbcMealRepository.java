...
public class JdbcMealRepository implements MealRepository {
    ...
    public Meal save(Meal meal, int userId) {
        ...
        if (meal.isNew()) {
            ...
            meal.setId(newKey.intValue());
                ...
                "UPDATE meal " +
                "SET date_time = :date_time, description = :description, calories = :calories, user_id = :user_id " +
                "WHERE id=:id", map) == 0) {
            return null;
        }
        return meal;
    ...
    public Meal get(int id, int userId) {
        final List<Meal> query = jdbcTemplate.query("SELECT * FROM meal WHERE id=? AND user_id = ?", ROW_MAPPER, id, userId);
